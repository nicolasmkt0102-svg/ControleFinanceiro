...
export default function App() {
  ...
  const [transactions, setTransactions] = useState(() => load("cf_transactions", []));
  const [messages, setMessages] = useState(() => {
    const saved = load("cf_messages", []);
    return saved.length > 0 ? saved : [{
      role: "assistant",
      text: "Olá! 👋 Bem-vindo ao seu **Controle Financeiro**. Registre receitas e despesas em linguagem natural.\n\nPode lançar assim:\n• _\"gastei 45 no mercado\"_\n• _\"recebi 3200 de salário\"_\n• _\"paguei 89 de Netflix\"_\n• _\"coloca 500 no cartão de crédito\"_\n\nO que vai lançar? 🤖",
      ts: Date.now(),
    }];
  });
  ...
  useEffect(() => { save("cf_transactions", transactions); }, [transactions]);
  useEffect(() => { save("cf_messages", messages.slice(-60)); }, [messages]);
  useEffect(() => { save("cf_budgets", budgets); }, [budgets]);
...
SUAS FUNÇÕES:
1. Detectar lançamentos na mensagem e retornar JSON de ação
2. Responder perguntas sobre o extrato
3. Dar dicas financeiras quando pertinente
...
FORMATO DE RESPOSTA OBRIGATÓRIO - retorne SEMPRE um JSON neste formato exato (sem markdown):
{
  ...
  "reply": "sua resposta amigável aqui com resumo atualizado se houver lançamento"
}
...
Seja muito conciso no reply (2-4 linhas). Use **negrito** apenas para valores importantes.`;
    ...
    try {
      ...
      if (parsed.action === "add_transaction" && parsed.transaction?.amount > 0) {
        const newTx = {
          ...
          person: parsed.transaction.person,
    ...
    <div style={{ minHeight: "100vh", background: COLORS.bg, color: COLORS.text, fontFamily: "'Inter', system-ui, sans-serif", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ background: COLORS.card, borderBottom: `1px solid ${COLORS.cardBorder}`, padding: "0 16px", display: "flex", alignItems: "center", position: "sticky", top: 0, zIndex: 10 }}>
        ...
        <div style={{ display: "flex", gap: 2 }}>
          {[["chat","💬 Chat"],["dashboard","📊 Dashboard"],["budget","🎯 Orçamento"]].map(([id, label]) => (
            <button key={id} onClick={() => setTab(id)} style={{ background: tab===id ? COLORS.emeraldDim : "transparent", color: tab===id ? COLORS.emerald : COLORS.textMuted, border: "none", borderRadius: 8, padding: "8px 12px", cursor: "pointer", fontWeight: tab===id ? 600 : 400, fontSize: 12, transition: "all .15s" }}>{label}</button>
          ))}
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "16px", display: "flex", flexDirection: "column", gap: 12 }}>
        {messages.map((m, i) => (
          <div key={i} style={{ display: "flex", justifyContent: m.role === "user" ? "flex-end" : "flex-start" }}>
            {m.role === "assistant" && (
              <div style={{ width: 28, height: 28, borderRadius: "50%", background: COLORS.emeraldDim, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, marginRight: 8, flexShrink: 0, marginTop: 2 }}>🤖</div>
            )}
            <div style={{ maxWidth: "80%", background: m.role === "user" ? COLORS.userBubble : COLORS.aiBubble, border: `1px solid ${m.role === "user" ? "#1a4a30" : COLORS.cardBorder}`, borderRadius: m.role === "user" ? "18px 18px 4px 18px" : "18px 18px 18px 4px", padding: "10px 14px", fontSize: 14, lineHeight: 1.5 }}>
              {m.transaction && (
                <div style={{ background: COLORS.emeraldDim, border: `1px solid ${COLORS.emerald}33`, borderRadius: 8, padding: "6px 10px", marginBottom: 8, fontSize: 12, color: COLORS.emerald }}>
                  ✅ {m.transaction.type === "income" ? "+" : "-"}{new Intl.NumberFormat("pt-BR",{style:"currency",currency:"BRL"}).format(m.transaction.amount)} · {m.transaction.description}
                </div>
              )}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <div style={{ background: COLORS.aiBubble, border: `1px solid ${COLORS.cardBorder}`, borderRadius: "18px 18px 18px 4px", padding: "12px 16px" }}>
              <div style={{ display: "flex", gap: 4 }}>
                {[0,1,2].map(i => <div key={i} style={{ width: 6, height: 6, borderRadius: "50%", background: COLORS.emerald, animation: `bounce 1s infinite ${i*0.15}s`, opacity: 0.7 }} />)}
              </div>
            </div>
          </div>
        )}
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", padding: 16, display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
        {[["+Entradas", new Intl.NumberFormat("pt-BR",{style:"currency",currency:"BRL"}).format(income), COLORS.emerald, COLORS.emeraldDim],
          ["-Saídas", new Intl.NumberFormat("pt-BR",{style:"currency",currency:"BRL"}).format(expense), COLORS.coral, COLORS.coralDim],
          ["Saldo", new Intl.NumberFormat("pt-BR",{style:"currency",currency:"BRL"}).format(balance), balance>=0?COLORS.emerald:COLORS.coral, balance>=0?COLORS.emeraldDim:COLORS.coralDim]
        ].map(([label,val,c,bg]) => (
            ...
            return (
              <div key={cat} style={{ marginBottom: 10 }}>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, marginBottom: 4 }}>
                  ...
                  <span style={{ color: COLORS.textMuted }}>{new Intl.NumberFormat("pt-BR",{style:"currency",currency:"BRL"}).format(val)} <span style={{ fontSize: 11 }}>({pct.toFixed(0)}%)</span></span>
                </div>
      ...
      <div style={{ background: COLORS.card, border: `1px solid ${COLORS.cardBorder}`, borderRadius: 14, overflow: "hidden" }}>
        ...
        {filtered.length === 0
          ? <div style={{ padding: 24, textAlign: "center", color: COLORS.textMuted, fontSize: 14 }}>Nenhum lançamento este mês.<br/>Use o chat ou o botão "Lançar" acima.</div>
          : filtered.map(t => {
              const c = CATEGORIES[t.category] || CATEGORIES.outros;
              return (
                <div key={t.id} style={{ display: "flex", alignItems: "center", padding: "12px 16px", borderBottom: `1px solid ${COLORS.cardBorder}`, gap: 12 }}>
                  ...
                  <div style={{ fontSize: 15, fontWeight: 600, color: t.type==="income" ? COLORS.emerald : COLORS.coral, flexShrink: 0 }}>{t.type==="income"?"+":"-"}{new Intl.NumberFormat("pt-BR",{style:"currency",currency:"BRL"}).format(t.amount)}</div>
          ...
          <div style={{ background: COLORS.card, border: `1px solid ${COLORS.cardBorder}`, borderRadius: "20px 20px 0 0", padding: 20, width: "100%", maxWidth: 480 }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
              {["expense","income"].map(t => (
                <button key={t} onClick={() => setForm(f => ({...f, type: t}))} style={{ flex: 1, background: form.type===t ? (t==="income" ? COLORS.emeraldDim : COLORS.coralDim) : COLORS.inputBg, border: `1px solid ${form.type===t ? (t==="income" ? COLORS.emerald : COLORS.coral) : COLORS.cardBorder}`, borderRadius: 10, padding: "10px", color: form.type===t ? (t==="income" ? COLORS.emerald : COLORS.coral) : COLORS.textMuted, cursor: "pointer", fontSize: 13, fontWeight: 600 }}>
                  {t==="income" ? "💰 Entrada" : "💸 Saída"}
                </button>
              ))}
            </div>
            {[["Descrição","text","description","Ex: Mercado, Netflix..."],["Valor (R$)","number","amount","0,00"],["Data","date","date",""],["Quem pagou? (opcional)","text","person","Ex: João, empresa..."]].map(([label,type,key,ph]) => (
            ...
            <label style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16, cursor: "pointer", fontSize: 13, color: COLORS.textDim }}>
              <input type="checkbox" checked={form.recurring} onChange={e => setForm(f => ({...f, recurring: e.target.checked}))} /> 🔄 Lançamento recorrente (assinatura)
            </label>
...
function BudgetView({ budgets, setBudgets, transactions, selectedMonth }) {
  ...
  function saveBudget() {
    ...
    const updated = editId
      ...
      : [...budgets, { id: Date.now(), ...form, amount: parseFloat(form.amount) }];
    setBudgets(updated);
    setShowAdd(false);
  }
    ...
    <div style={{ flex: 1, overflowY: "auto", padding: 16, display: "flex", flexDirection: "column", gap: 16 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          ...
          <div style={{ fontSize: 12, color: COLORS.textMuted, marginTop: 2 }}>Previsão de gastos do mês</div>
        </div>
      ...
      {budgets.length === 0 ? (
        ...
        const pct = b.amount>0 ? Math.min((spentAmt/b.amount)*100,100) : 0;
        const over = spentAmt>b.amount;
        const warn = !over && pct>=80;
        const left = b.amount - spentAmt;
        return (
          <div key={b.id} style={{ background: COLORS.card, border: `1px solid ${over ? COLORS.coral+"44" : COLORS.cardBorder}`, borderRadius: 14, padding: 14 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontWeight: 600, fontSize: 14, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{b.description}</div>
                <div style={{ fontSize: 12, color: COLORS.textMuted }}>{cat.label}</div>
              </div>
              <div style={{ textAlign: "right", flexShrink: 0 }}>
                <div style={{ fontSize: 14, fontWeight: 700 }}>{fmt(b.amount)}</div>
                <div style={{ fontSize: 11, color: COLORS.textMuted }}>previsto</div>
              </div>
            ...
            <div style={{ height: 6, background: COLORS.inputBg, borderRadius: 3, overflow: "hidden", marginBottom: 6 }}>
              <div style={{ height: "100%", width: `${pct}%`, background: over ? COLORS.coral : warn ? COLORS.amber : COLORS.emerald, borderRadius: 3 }} />
            </div>
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12 }}>
              <span style={{ color: COLORS.textMuted }}>Gasto: <strong style={{ color: COLORS.text }}>{fmt(spentAmt)}</strong></span>
              {over ? <span style={{ color: COLORS.coral, fontWeight: 600 }}>⚠️ {fmt(Math.abs(left))} acima</span>
               : warn ? <span style={{ color: COLORS.amber, fontWeight: 600 }}>⚡ {fmt(left)} restante</span>
               : <span style={{ color: COLORS.emerald }}>{fmt(left)} restante</span>}
            </div>
          </div>
        );
      })}
      ...
      {showAdd && (
        <div style={{ position: "fixed", inset: 0, background: "#000A", display: "flex", alignItems: "flex-end", justifyContent: "center", zIndex: 50 }} onClick={() => setShowAdd(false)}>
          <div style={{ background: COLORS.card, border: `1px solid ${COLORS.cardBorder}`, borderRadius: "20px 20px 0 0", padding: 20, width: "100%", maxWidth: 480 }} onClick={e => e.stopPropagation()}>
            ...
            {[["Descrição","text","description","Ex: Supermercado do mês, Aluguel..."],["Valor previsto (R$)","number","amount","0,00"]].map(([label,type,key,ph]) => (
